...
用于低速视频演示时，可选参数 `torque_soft_velocity_start_rad_s` 和
`torque_soft_velocity_limit_rad_s` 分别表示起动判定速度和监管目标速度。第一次
起动前保持完整请求力矩；检测到速度越过起动阈值后，锁定进入平滑速度监管，
即使随后被外力阻挡也不会重新施加完整起动力矩。启用后监管阶段不再是严格
恒力矩；`max_velocity_rad_s` 硬停机保护仍然有效，并且必须高于监管目标速度。

若同时设置 `torque_soft_brake_gain_nm_per_rad_s` 和
`torque_soft_brake_max_nm`，监管阶段使用连续比例律计算正向恢复力矩或有限反向
制动力，避免在完整正力矩和固定反向力矩之间来回切换。
`torque_velocity_filter_alpha` 对速度反馈进行低通滤波，减少低速编码器噪声导致
的力矩跳变。主动监管仅用于受控演示，不能替代硬件急停。

`torque_speed_feedforward_nm` 用于补偿连续转动时的齿轮和轴承摩擦。若该值为
零，比例调节在达到目标速度时也会把力矩降为零，可能产生“停下—满力矩再起动”
的极限环；合理的前馈值可让目标速度附近仍保留维持运动所需的基础力矩。

`torque_demo_duration_s` 为纯力矩录像提供独立计时停机，从收到第一条有效力矩
...
ros2 launch rs03_current_torque_control rs03_current_torque.launch.py \
  ...
  position_slew_rate_rad_s:=0.05 position_ramp_max_error_rad:=0.03 \
...
`position_slew_rate_rad_s` 还会在 ROS2 节点内把较大的位置阶跃转换为小步目标；
`position_ramp_max_error_rad` 在电机因静摩擦没有跟上时暂停斜坡，防止位置目标持续
跑远、积累较大力矩后突然突破静摩擦。该斜坡是 PP 内部速度规划之外的额外安全层。
